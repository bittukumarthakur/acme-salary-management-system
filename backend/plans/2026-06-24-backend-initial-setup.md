...
Set up a minimal Node.js + TypeScript + Express backend with a health endpoint on port 8080, using standard practices (npm init, tsx watch for dev, tsc for build, prettier for formatting).
...
5. Create `src/main.ts` — Express server on port 8080 with `GET /health` → `{ status: "ok" }`
...
9. Verify: `npm run dev` starts server, `curl localhost:8080/health` returns `{"status":"ok"}`
